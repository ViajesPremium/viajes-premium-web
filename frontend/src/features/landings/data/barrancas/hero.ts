import type { LandingHero } from "./../types";

export const BARRANCAS_PREMIUM_HERO: LandingHero = {
  seoHeading: "Barrancas Premium",
  title: {
    wordOne: "Visita",
    wordTwo: "Barrancas",
    wordThree: "en Clase",
    wordFour: "Premium",
  },
  description:
    "Eleva tu vida recorriendo uno de los paisajes más impresionantes de México con naturaleza, gastronomía y aventura en Clase PREMIUM.",
  ctaPrimary: {
    label: "Solicita tu cotización",
    target: "#second-form",
  },
  heroOverlay: {
    baseImage: "/media/landings/barrancas/hero/mujer.webp",
    overlayImage: "/media/landings/barrancas/hero/hombre.webp",
    baseAlt: "Barrancas Premium",
    overlayAlt: "Barrancas Premium en detalle",
  },
};
